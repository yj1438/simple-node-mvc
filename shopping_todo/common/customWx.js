...
function CustomWx(self) {
    ...
    try {
        ...
        this.domain = 'https://www.babytree.com';
    }
};
...
CustomWx.prototype.req = function (options, noLogin) {
    ...
    requestData.version = '6.3.2';              // 加入默认版本号
    requestData.is_wechat = 1;                  // 加入一个默认的参数 is_wechat=1
    requestData.source_code = 1;                // 针对小程序的统计参数
    if (noLogin) {
    ...
    return new Promise((resolve, reject) => {
        if (this.isGetLoginString) {
            ...
            this.app.getUserInfo((info) => {
                 ...
                 */
                this.app.getWx().req({
                    // url: '/api/wx_app_lama_lite/login',
                    ...
                    success: (res) => {
                        ...
                        if (result.status && result.status === 'success') {
                            // 获取到 loginString
                            const loginString = result.data.login_string || result.data.openId;
...
CustomWx.prototype.webpToJpg = function(data) {
    if(!data){
        return;
    }
