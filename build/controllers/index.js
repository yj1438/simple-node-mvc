...
function makeUaData(userAgent, callback) {
    ...
    var UA_PARSE_URL = 'http://www.useragentstring.com/?uas=' + encodeURIComponent(userAgent) + '&getJSON=all';
    ...
    request(UA_PARSE_URL, function (error, response, body) {
        if (!error && response.statusCode === 200) {
            ...
            //处理特殊 UA
            var uaMatch = userAgent.match(/(UCBrowser)\/([\d.]+)/) //UC
...
exports.index = function () {
    ...
    if (uaDataStore.has(hashId)) {
        ...
        http2Service.findByHashid(hashId, function (err, rows) {
            if (err) {
                ...
                if (!rows || rows.length === 0) {
                     ...
                     * 所以有了uaDataStore
                     */
                    makeUaData(userAgent, function (uaData) {
                        if (!uaData) {
                            uaDataStore.remove(hashId);
                            return;
                        }
                        ...
                        uaData.is_spdy = _this.req.isSpdy ? 1 : 0;
                        uaData.protocol = protocolMap[_this.req.spdyVersion];
                        uaData.ctr = uaDataStore.getCTR(hashId);
                        http2Service.add(uaData, function (err, result) {
                            if (err) {
                                console.log(err);
                            } else {
