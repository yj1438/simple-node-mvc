...
exports.index = function () {
    ...
    //ua hash 进行过滤
    fs.readFile('data/hash.json', 'utf8', function (err, data) {
        if (err) {
            throw err;
            return;
        }
        if (!data || data.indexOf(hashId) === -1) {
            ...
            request(UA_PARSE_URL, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    var uaData = makeUaData(JSON.parse(body), userAgent);
                    uaData.hashId = hashId;
                    uaData.isSpdy = self_req.isSpdy ? 'yes' : 'no';
                    uaData.protocol = protocolMap[self_req.spdyVersion];
                    //写信息
                    fs.appendFile('data/ua_data.json', (0, _stringify2.default)(uaData) + '\n', 'utf8', function (err) {
                        if (err) throw err;
                    });
                    //写 hashId
